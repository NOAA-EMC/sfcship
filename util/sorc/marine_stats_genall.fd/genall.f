c     PROGRAM GENALL
C     PURPOSE: This program generates stats to be forwarded to UKMET
C     each month. UKMET is the lead center for marine statistics.
C
...
c               04 Feb 03 ccm adding code to read total number of reports,
C                 total number of 'ship' reports, and print the month/year
C                 and trailing text to this program instead of having to
C                 do it manually.
...
C                 char string.
C               18 Aug 04 ccm adding one blank line to top of output so <PRE>
C                 tag in www html formats stats file correctly.
C*******************************************************************
      PROGRAM GENALL
      ...
      CHARACTER*33 CENDHDR1
      CHARACTER*22 CENDHDR2
      CHARACTER*36 CENDHDR3
      CHARACTER*18 CENDHDR4
      CHARACTER*61 CENDHDR5
      CHARACTER*24 CENDHDR6
C
      ...
      INTEGER LU1, LU2, LU3
      INTEGER IDAT(8), ICMONLEN(12), NUMREP, NUMREPFNL
      INTEGER NUMSHIPOBS
C
      REAL SUMD(8), SUMSQ(8)
      REAL SD(8), DMEAN(8)
      REAL DIF(8)
      REAL SVDIF(50000,8), ENDLAT, ENDLON
      real savsd, savbias
...
C
      DATA IEND/0/,NN/0/,KTDUP/0/
      DATA IHDR(01:40)/'          NCEP CENTRAL OPERATIONS (NCO) '/
      DATA IHDR(41:80)/'PLATFORM STATS'/
      DATA IHDR1(01:40)/'PLATFORM   TYPE   NO. REPORTS   MEAN DIF'/
      DATA IHDR1(41:80)/'F   SD DIFF   # GROSS ERR'/
      DATA CENDHDR1(01:33)/'TOTAL NUMBER OF OBSERVATIONS FOR '/
      DATA CENDHDR2(01:22)/' (EXCLUDING DUPLICATES'/
      DATA CENDHDR3(01:36)/'AND THOSE FROM CALLSIGN ''SHIP'') WAS '/
      DATA CENDHDR4(01:18)/'.  TOTAL NUMBER OF'/
      DATA CENDHDR5(01:34)/'OBSERVATIONS FROM CALLSIGN ''SHIP'' '/
      DATA CENDHDR5(35:61)/'(EXCLUDING DUPLICATES) WAS '/
      DATA CENDHDR6(01:24)/'TOTAL NUMBER OF IDS WAS '/
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
 ...
 1000 FORMAT(A80)
      ...
      ISTA = 1
C-----------------------------------------------------------------------
C     Read in total number of reports in archive file and read in total
C     number of reports from callsign 'SHIP'.  Compute the difference so
C     that it may be written out at the end of the SST output file.
C-----------------------------------------------------------------------
       READ(LU2,1004) NUMREP
       READ(LU3,1004) NUMSHIPOBS
 1004  FORMAT(I8)
       NUMREPFNL = NUMREP - NUMSHIPOBS 
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      NREP = 0
      KCOUNT=0
   25 READ(LU1,1002,END=100) ID,ITYPE,LAT,LON,CDATE,ND(1),NV(1),ND(2),
...
C-----------------------------------------------------------------------
      ...
      IF(K.NE.0) GO TO 25
C-----------------------------------------------------------------------
C         SKIP REPORTS W/ LONG BETWEEN PRIME MERIDIAN AND 1 W (temporary)
C-----------------------------------------------------------------------
      IF ((LON.GT.0.AND.LON.LT.100).and.CDATE.lt.'200304010301') 
     +    GO TO 25
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      IF(ID.NE.IDT) THEN
        PRINT *,' FOUND NEW ID ',ID,'  LAST ID WAS ',IDT, K
...
C  PRINT SLP.
      DO 31 JJ = 1,5
        ...
        TOTOBS(JJ) = ITOTOB(JJ)
        BAD(JJ) = IBAD(JJ)
        IF(ITOTOB(JJ).GE.20) THEN
          IF(JJ.EQ.1) THEN
C           SLP
            IF(ABS(DMEAN(JJ)).GE.4.0.OR.SD(JJ).GE.6.0.OR.
     1         (BAD(JJ)/TOTOBS(JJ)).GE.0.25) THEN
...
C           AIR TEMP.
     ...
     1        DMEAN(JJ),SD(JJ),IBAD(JJ)
          ...
          ELSEIF(JJ.EQ.3) THEN
C           WIND DIR.
            IF(ABS(DMEAN(JJ)).GE.30.0.OR.SD(JJ).GE.80.0.OR.
     1         (BAD(JJ)/TOTOBS(JJ)).GE.0.25) THEN
...
C------------------------------------------------------------------
      WRITE(44,777) CBLNK
      WRITE(44,993) CENDHDR1,CMON(IMON),CYR,CENDHDR2
 993  FORMAT(A,A,1X,A4,A)
      WRITE(44,994) CENDHDR3,NUMREPFNL,CENDHDR4
 994  FORMAT(A,1X,I7,A)
      WRITE(44,995) CENDHDR5,NUMSHIPOBS
 995  FORMAT(A,1X,I6,'.')
      WRITE(44,996) CENDHDR6,ISTA      
 996  FORMAT(A,1X,I6,'.')
