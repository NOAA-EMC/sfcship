c     PROGRAM GENISS
C     PURPOSE: THIS PROGRAM GENERATES BRACKNELL FORMAT STATS FOR THE
C     INTERNATIONAL SEAKEEPERS SOCIETY VESSELS.               
C
...
c               15 May 01 ccm modifying for International SeaKeepers.   
c               03 Feb 03 ccm Adding code to write month/year and trailing
c                 lines within this program instead of doing so manually.
c               18 Aug 04 ccm Adding one blank line to top of output file
c                 so <pre> tag formats html stats correctly.
C*******************************************************************
      PROGRAM GENISS
      ...
      CHARACTER*66 CENDHDR1
      CHARACTER*51 CENDHDR2
      CHARACTER*43 CENDHDR3
      CHARACTER*8 CENDHDR4
      CHARACTER*17 CENDHDR5
C
...
C
      DATA IEND/0/,KTDUP/0/
      DATA IHDR(01:40)/' NCEP CENTRAL OPERATIONS (NCO) PLATFORM '/
      DATA IHDR(41:80)/'STATS - INTERNATIONAL SEAKEEPERS SOCIETY'/
      DATA IHDR1(01:40)/'PLATFORM   TYPE   NO. REPORTS   MEAN DIF'/
      DATA IHDR1(41:80)/'F   SD DIFF   # GROSS ERR'/
      DATA CENDHDR1(01:40)/'TOTAL NUMBER OF INTERNATIONAL SEAKEEPERS'/
      DATA CENDHDR1(41:66)/' SOCIETY OBSERVATIONS FOR '/
      DATA CENDHDR2(01:33)/' (EXCLUDING DUPLICATES AND KSxxx '/
      DATA CENDHDR2(34:51)/'OBSERVATIONS FROM '/
      DATA CENDHDR3(01:39)/'BULLETIN ORIGINATORS OTHER THAN ''KWUM'')'/
      DATA CENDHDR3(40:43)/' WAS'/
      DATA CENDHDR4(01:08)/'.  TOTAL'/
      DATA CENDHDR5(01:17)/'NUMBER OF IDS WAS'/
C-----------------------------------------------------------------------
...
C------------------------------------------------------------------
      WRITE(44,777) CBLNK
      WRITE(44,993) CENDHDR1
 993  FORMAT(A)
      WRITE(44,994) CMON(IMON),CYR,CENDHDR2
 994  FORMAT(A,1X,A4,A)
      WRITE(44,995) CENDHDR3,NUMREP,CENDHDR4
 995  FORMAT(A,1X,I6,A)
      WRITE(44,996) CENDHDR5,ISTA
 996  FORMAT(A,1X,I2,'.')
